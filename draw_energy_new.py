...
for platform in ["mi9"]:
    ...
    for i, ((device, lats), (_device, powers)) in enumerate(zip(df.iteritems(), power_df.iteritems())):
        ...
        ppe = [1000 / (power * lat * lat) for power, lat in zip(powers, lats)]
        for energy, x in zip(ppe, xbr):
            if energy > ylim:
                plt.text(x, ylim + 1, "{:.3f}".format(energy), horizontalalignment="center")
        index = device_index[device]
        plt.bar(xbr, energies, color=colors[index], hatch=hatches[index], width=bar_width, label=device_dict[device], edgecolor="grey")
        xbr = xbr + bar_width
